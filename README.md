...
- A release is a `v*.*.*` tag. `.github/workflows/release.yml` refuses a tag whose version differs from the code or is not on `main`, repeats the checks on the package and only then deploys it to WordPress.org; a last job checks that the published zip holds the same files, byte for byte, as the one it inspected.
- How a release is made: [`RELEASE-CHECKLIST.md`](RELEASE-CHECKLIST.md) (in Spanish). It has ten steps, in order: the major-number question, the version in its three places, «Tested up to», CI on `main`, the CI zip opened and read, the pinned actions re-read, Fernando's go-ahead, the tag, the watch window (including the zip WordPress.org actually serves), and the rollback said plainly. Until the `deploy` job runs in a protected environment, publishing is guarded by that process only.
- The checks live in `bin/release_checks.py` (Python 3.10+, standard library only; `selftest` proves each check can fail). `bin/`, `tests/`, `.github/`, `.wordpress-org/`, this README and the checklist never reach the package.

**The major-number promise.** The major number of the plugin version is the protocol contract between the plugin and the API. The API stops analysing a site (and its card in the panel says «Actualiza el plugin») only when the installed major number is lower than the major number of the latest version published on WordPress.org. A difference within the same major number is analysed normally, and the card says so with a quiet line, «Hay una versión nueva del plugin.», withholding nothing. That is why any protocol change (the envelope, its fields, the encryption or the registration) ships as a major version, and a major version is never published for any other reason. If WordPress.org does not answer or its version cannot be read, no gate applies. The binding text, in Spanish, is step 1 of the checklist.
